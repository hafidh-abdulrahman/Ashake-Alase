...
export default function ConfirmationPage() {
  ...
  if (!order) return <EmptyState title="We can't find that order" text="Check the order number and try again." actionLabel="Back to home" actionTo="/" />
      ...
      <div className="mx-auto max-w-3xl">
        <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center">
          ...
          <div>
            ...
            <p className="mt-2 text-lg text-ink-soft">Thank you, {order.customer.fullName.split(' ')[0]}. We have received your order and receipt.</p>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4 rounded-3xl bg-ink p-6 text-surface">
          <div>
            ...
            <p className="font-display text-3xl font-extrabold tracking-wide">{order.orderNumber}</p>
          </div>
          <button type="button" onClick={copy} className="inline-flex items-center gap-2 rounded-full border-2 border-surface/40 px-4 py-2 text-sm font-semibold transition-colors hover:bg-surface hover:text-ink">
            {copied ? <Check className="size-4" aria-hidden /> : <Copy className="size-4" aria-hidden />}
            {copied ? 'Copied' : 'Copy'}
          </button>
        </div>

        <div className="mt-6 rounded-3xl bg-warn-bg p-6 text-warn" role="status">
          <div className="flex flex-wrap items-center gap-3">
            <Info className="size-5" aria-hidden />
            <h2 className="text-xl font-bold">{paymentStatusLabel[order.payment.status]}</h2>
          </div>
          <p className="mt-2">
            Uploading your receipt does not mean your payment is verified. We will check your transfer, then confirm your order. Keep your order number handy for any questions.
          </p>
        </div>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          <section aria-labelledby="summary" className="rounded-3xl border border-line bg-paper p-6">
          ...
          <section aria-labelledby="delivery" className="rounded-3xl border border-line bg-paper p-6">
            ...
            <dl className="space-y-3.5">
              ...
              <div>
                ...
                <dd className="font-semibold">{formatPlainDate(order.delivery.preferredDate)}</dd>
              </div>
