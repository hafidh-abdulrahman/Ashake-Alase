...
const clampQty = (product: Product, qty: number) => {
  const cap = Math.min(product.maxPerOrder, product.availableQuantity ?? Infinity)
  return Math.max(1, Math.min(qty, cap))
}

export function CartProvider({ children }: { children: ReactNode }) {
  const [raw, setRaw] = useState<CartLine[]>(() => readJson<CartLine[]>(CART_KEY, []))
  const [draft, setDraft] = useState<CheckoutDraft>(() => ({ ...emptyDraft, ...readJson<Partial<CheckoutDraft>>(DRAFT_KEY, {}) }))
  ...
  const addItem = useCallback(
    (productId: string, quantity: number) => {
      ...
      setRaw((prev) => {
        ...
        if (existing) {
          return prev.map((l) => (l.productId === productId ? { ...l, quantity: clampQty(product, l.quantity + quantity) } : l))
        }
  ...
  const setQuantity = useCallback(
    (productId: string, quantity: number) => {
      const product = productById.get(productId)
      if (!product) return
      setRaw((prev) => prev.map((l) => (l.productId === productId ? { ...l, quantity: clampQty(product, quantity) } : l)))
    },
    [productById],
  )

  const removeItem = useCallback((productId: string) => setRaw((prev) => prev.filter((l) => l.productId !== productId)), [])
  ...
  const value: CartValue = {
    lines, itemCount, subtotal, ready, addItem, setQuantity, removeItem, clear,
    draft, updateDraft, areas, selectedArea, deliveryFee, total,
