...
Copy `.env.example` to `.env` to change contact details.
...
| `/payment`                                         | Secure online payment step     |
...
src/
  ...
  config/site.ts  Brand name, navigation, contact details (from env)
  ...
  context/        CartContext (cart and checkout draft)
...
2. Rewrite the functions in `src/services/*` to call Supabase. Signatures stay the same, so no component changes.
3. Set `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` (see `src/lib/supabase.ts`).
4. Add Supabase Auth and enforce it in `src/components/admin/AdminGuard.tsx`, plus row level security so only admins can list or update orders.
5. Create orders through a database function (RPC) so prices and delivery fees are calculated on the server, never trusted from the browser.
6. Remove `simulateLatency` in `src/services/delay.ts` and the demo reset in the admin dashboard.
7. Optional: Paystack or Flutterwave. Add the initialise/verify integration when the provider is selected and set `payment.method` accordingly.
